...
test('preview switches between the steps and course-rooted panorama without editing the draft',async()=>{
 ...
 const {createKnowledgeGraph}=await import('../src/knowledge/model.mjs');const knowledge=createKnowledgeGraph();
 const point=knowledge.nodes.find(n=>n.name==='工业机器人');
 assert.ok(point);project.stages[0].tasks[0].contents.push({id:'linked-point',knowledgeNodeId:point.id});
 const errors=[];const vc=new VirtualConsole();vc.on('jsdomError',e=>errors.push(e.message));
 ...
 try{
  ...
  assert.ok(graph.querySelectorAll('.panorama-edges .is-dimmed').length>0);
  activity.dispatchEvent(new dom.window.KeyboardEvent('keydown',{key:'Escape',bubbles:true}));await wait(()=>!graph.querySelector('.is-dimmed'));
  activity.click();await wait(()=>activity.getAttribute('aria-pressed')==='true');activity.click();await wait(()=>!graph.querySelector('.is-dimmed'));
  node.click();await wait(()=>node.getAttribute('aria-pressed')==='true');
  graph.querySelector('.panorama-world').click();await wait(()=>!graph.querySelector('.is-dimmed'));
  node.click();await wait(()=>node.getAttribute('aria-pressed')==='true');
