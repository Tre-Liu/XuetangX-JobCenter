...
async function mount(fixture,saved={}){
 const dom=new JSDOM(`<div id="root"></div><script>${bundle.outputFiles[0].text.replaceAll('</script','<\\/script')}</script>`,{url:'http://localhost/',runScripts:'dangerously',beforeParse(w){w.structuredClone=structuredClone;w.fixture=fixture;for(const [k,v] of Object.entries(saved))w.localStorage.setItem(k,JSON.stringify(v));}});
 ...
 await wait(()=>d.querySelector('.gen-context-grid')); // Form appears only after the Markdown prompt finishes.
