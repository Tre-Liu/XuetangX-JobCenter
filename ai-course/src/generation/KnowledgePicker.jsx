...
import {MAX_KNOWLEDGE_POINTS} from './knowledge-matching.mjs';
import {createPortal} from 'react-dom';
import {Icon} from '../components/Icon';

export function KnowledgePicker({task,nodes,selected,onConfirm,onClose}){
 const [draft,setDraft]=useState(()=>[...new Set(selected)].filter(id=>nodes.some(node=>node.id===id)).slice(0,MAX_KNOWLEDGE_POINTS));
 ...
 const toggle=id=>setDraft(values=>values.includes(id)?values.filter(value=>value!==id):values.length<MAX_KNOWLEDGE_POINTS?[...values,id]:values);
 ...
 const renderNode=node=>{
  ...
  const row=<label className="knowledge-tree-label"><input type="checkbox" aria-label={`选择${node.name}`} checked={draft.includes(node.id)} disabled={!draft.includes(node.id)&&draft.length>=MAX_KNOWLEDGE_POINTS} onChange={()=>toggle(node.id)}/><span>{node.name}</span><small>{node.level}级</small></label>;
 ...
 return createPortal(<div className="knowledge-picker-backdrop" onMouseDown={event=>{if(event.target===event.currentTarget)onClose();}}>
  <section className="knowledge-picker-drawer" ref={panel} role="dialog" aria-modal="true" aria-label="选择知识点">
   ...
   <div className="knowledge-picker-task"><small>当前任务</small><strong>{task.title}</strong><p>按图谱层级展开并勾选，选择上级不会自动选择下级，每个任务最多选择 10 个知识点。</p></div>
   <div className="knowledge-picker-tree">{nodes.filter(node=>!parentOf(node)).map(renderNode)}{!nodes.length&&<p>暂无可用知识点</p>}</div>
   <footer><span>已选 {valid.length} / {MAX_KNOWLEDGE_POINTS} 个知识点</span><button className="outlined" onClick={onClose}>取消</button><button className="primary" onClick={()=>onConfirm(valid)}>确认选择</button></footer>
  </section>
 </div>,document.body);
}
