...
export function KnowledgeMatching({project,graph,enabled,choice,onChoice,selections,onSelections}){
 ...
 {choice==='yes'&&nodes.length>0&&<><p className="gen-notice">已为每个任务默认勾选最多 10 个可用知识点，可取消勾选，也可从知识图谱选择节点重新匹配。只关联所选知识点及其直接学习单元。</p><div className="gen-match-tasks">{project.stages.map((stage,index)=><details open={index===0} className="gen-match-stage" key={stage.id} aria-label={`阶段 ${index+1}：${stage.title}`}><summary className="gen-match-stage-heading"><span>阶段 {index+1}</span><h4>{stage.title}</h4><small>{stage.tasks.length} 个任务</small></summary>{stage.tasks.map(task=><details key={task.id} open><summary>{task.title}<small>已选 {(selections[task.id]||[]).filter(id=>nodes.some(n=>n.id===id)).length} / 10 个知识点</small></summary><button className="outlined gen-graph-picker" aria-expanded={picking===task.id} aria-haspopup="dialog" onClick={event=>{event.currentTarget.focus();setPicking(task.id);}}>选择知识点</button><div className="gen-match-nodes">{nodes.filter(node=>(selections[task.id]||[]).includes(node.id)).map(node=><label className="gen-ability" key={node.id}><input type="checkbox" aria-label={`匹配${node.name}`} checked={(selections[task.id]||[]).includes(node.id)} onChange={()=>toggle(task.id,node.id)}/><span>{node.name}<small>{node.level}级知识点</small></span></label>)}{!(selections[task.id]||[]).some(id=>nodes.some(node=>node.id===id))&&<p>暂无已匹配知识点，可点击“选择知识点”添加。</p>}</div></details>)}</details>)}</div></>}
