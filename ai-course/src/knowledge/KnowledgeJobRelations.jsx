...
export function KnowledgeJobRelations({graph,projects,nodeId,onRemove,onProject}){
 ...
 return <section className="kg-job-relations" aria-label="岗位关系列表">
  ...
  {rows.some(row=>row.demo)&&<p className="kg-job-demo-note">含模拟数据，用于演示岗位关系，可逐条删除。</p>}
  ...
  </li>)}</ul>:<div className="kg-detail-empty"><Icon name="users" size={36}/><h3>暂无岗位关系</h3><p>在项目任务中关联当前知识点与岗位能力项后，将在此显示对应关系。</p><button className="outlined" onClick={onProject}>进入项目式学习</button></div>}
 </section>;
}
