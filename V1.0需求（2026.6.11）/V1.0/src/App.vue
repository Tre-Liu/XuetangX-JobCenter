...
type IndustryResearchStoredState = {
  initialized?: boolean
  jobStatuses?: Record<string, IndustryResearchJobStatus>
  selectedAt?: string
}
type IndustryResearchJobStatus = 'active' | 'inactive'
const createDefaultIndustryResearchJobStatuses = (): Record<string, IndustryResearchJobStatus> =>
  Object.fromEntries(PORTRAIT_JOB_PROFILES.map((job) => [job.id, 'active']))
const readIndustryResearchStoredState = (): IndustryResearchStoredState | null => {
  if (typeof window === 'undefined') return null
  try {
    const raw = window.localStorage.getItem(industryResearchStateKey)
    return raw ? JSON.parse(raw) as IndustryResearchStoredState : null
  } catch {
    return null
  }
}
const readIndustryResearchDemoInitialized = () => {
  return readIndustryResearchStoredState()?.initialized === true
}
const readIndustryResearchJobStatuses = () => ({
  ...createDefaultIndustryResearchJobStatuses(),
  ...(readIndustryResearchStoredState()?.jobStatuses ?? {})
})
...
const industryResearchJobStatuses = ref<Record<string, IndustryResearchJobStatus>>(readIndustryResearchJobStatuses())
const industryResearchDemoInitialized = ref(readIndustryResearchDemoInitialized())
const industryResearchCurrentPage = ref(1)
const industryResearchPageSize = 8
const industryResearchJobSearchText = ref('')
...
const currentJobResearchMode = ref<'industry' | 'job'>('job')
const expandableJobMenuItems = ['产业调研', '产业调研报告'] as const
...
const filteredIndustryResearchJobs = computed(() => {
  const keyword = industryResearchJobSearchText.value.trim().toLowerCase()
  if (!keyword) return PORTRAIT_JOB_PROFILES
  return PORTRAIT_JOB_PROFILES.filter((job) => job.name.toLowerCase().includes(keyword))
})
const industryResearchTotalPages = computed(() =>
  Math.max(1, Math.ceil(filteredIndustryResearchJobs.value.length / industryResearchPageSize))
)
const industryResearchJobPageNumbers = computed(() =>
  Array.from({ length: industryResearchTotalPages.value }, (_, index) => index + 1)
)
const paginatedIndustryResearchJobs = computed(() => {
  const start = (industryResearchCurrentPage.value - 1) * industryResearchPageSize
  return filteredIndustryResearchJobs.value.slice(start, start + industryResearchPageSize)
})
...
const industryResearchCmsInitializationUrl = () => {
  if (typeof window !== 'undefined' && window.location.protocol === 'file:') {
    return new URL('./index.html?view=industry-research-admin', window.location.href).toString()
  }
...
const startIndustryResearchInitialization = () => {
  ...
  industryResearchJobStatuses.value = createDefaultIndustryResearchJobStatuses()
  industryResearchCurrentPage.value = 1
  industryResearchJobSearchText.value = ''
...
const persistIndustryResearchSelection = () => {
  ...
  window.localStorage.setItem(industryResearchStateKey, JSON.stringify({
    initialized: true,
    jobStatuses: industryResearchJobStatuses.value,
...
const handleIndustryResearchStorage = (event: StorageEvent) => {
  if (event.key === industryResearchStateKey) {
    refreshIndustryResearchDemoInitialized()
    industryResearchJobStatuses.value = readIndustryResearchJobStatuses()
    industryResearchJobSearchText.value = ''
  }
}
const setIndustryResearchJobStatus = (jobId: string, status: IndustryResearchJobStatus) => {
  industryResearchJobStatuses.value = {
    ...industryResearchJobStatuses.value,
    [jobId]: status
  }
  persistIndustryResearchSelection()
...
const searchIndustryResearchJobs = () => {
  industryResearchCurrentPage.value = 1
}
watch(industryResearchJobSearchText, () => {
  industryResearchCurrentPage.value = 1
})
...
const setJobMenuExpanded = (item: ExpandableJobMenuItem, expanded: boolean) => {
  expandedJobMenus.value = {
    ...
    产业调研报告: item === '产业调研报告' ? expanded : false
  }
}
const selectJobSection = (item: string) => {
  const shouldToggleMenu = isExpandableJobMenuItem(item)
  const nextExpanded = shouldToggleMenu ? !expandedJobMenus.value[item] : false
  currentJobSection.value = item
  if (item === '产业调研') currentJobResearchMode.value = 'job'
  if (item === '产业调研报告') currentReportView.value = 'library'
  if (shouldToggleMenu) setJobMenuExpanded(item, nextExpanded)
  selectedJobId.value = ''
  activeDetailTab.value = 'basic'
  closePortraitJobDialog()
}
...
const openReportLibrary = () => {
  ...
  setJobMenuExpanded('产业调研报告', true)
        ...
        <article class="cms-management-card">
          ...
          <section class="cms-industry-panel">
            <header class="cms-industry-head">
              <div>
                <h2>产业调研管理</h2>
                <p>用于开通当前专业的产业调研能力，初始化完成后生成岗位清单供管理员维护生效状态。</p>
              </div>
              <button
            ...
            <section v-if="industryResearchStatus === 'idle'" class="cms-init-empty">
              <div>
                <strong>待初始化</strong>
                <p>点击“数据初始化”后，系统将根据专业基础信息、培养方向、岗位资料与已有建设数据生成岗位清单。</p>
              </div>
            </section>
            ...
            <section v-else-if="industryResearchStatus === 'initializing'" class="industry-initialization-progress cms-init-progress">
              <div class="industry-progress-copy">
                <span>初始化中</span>
                <strong>正在根据专业名称、培养方向、岗位资料和已有专业数据生成岗位清单</strong>
                <p>正在识别岗位名称、能力关键词、招聘需求和岗位建设状态。</p>
              </div>
            ...
            <template v-else>
              <div class="cms-chain-toolbar">
                <div>
                  <h3>岗位列表</h3>
                  <p>初始化生成当前专业全部岗位，管理员可在操作列设置岗位生效或失效。</p>
                </div>
                <span class="cms-job-total">共 {{ filteredIndustryResearchJobs.length }} 个岗位</span>
              </div>

              <div class="cms-job-search-row">
                <label class="cms-job-search">
                  <span>岗位名称</span>
                  <input
                    v-model="industryResearchJobSearchText"
                    type="search"
                    placeholder="输入岗位名称搜索"
                    @keyup.enter="searchIndustryResearchJobs"
                  >
                </label>
                <button type="button" @click="searchIndustryResearchJobs">搜索</button>
              </div>

              <div class="cms-job-table-wrap">
                <table class="cms-job-table">
                  <thead>
                    <tr>
                      <th>岗位名称</th>
                      <th>操作</th>
                  ...
                  <tbody>
                    <tr v-if="paginatedIndustryResearchJobs.length === 0">
                      <td colspan="2" class="cms-job-empty-cell">未找到匹配岗位</td>
                    </tr>
                    <tr v-for="job in paginatedIndustryResearchJobs" :key="job.id">
                      <td>{{ job.name }}</td>
                      <td>
                        <div class="cms-job-status-actions">
                          <button
                            type="button"
                            :class="{ active: industryResearchJobStatuses[job.id] === 'active' }"
                            @click="setIndustryResearchJobStatus(job.id, 'active')"
                          >
                            生效
                          </button>
                          <button
                            type="button"
                            :class="{ active: industryResearchJobStatuses[job.id] === 'inactive' }"
                            @click="setIndustryResearchJobStatus(job.id, 'inactive')"
                          >
                            失效
                          </button>
                        </div>
              ...
              <footer class="cms-chain-footer">
                <span>第 {{ industryResearchCurrentPage }} / {{ industryResearchTotalPages }} 页</span>
                <nav class="cms-pagination" aria-label="岗位列表分页">
                  <button
                    type="button"
                  ...
                  <button
                    v-for="page in industryResearchJobPageNumbers"
                    :key="page"
                    type="button"
          ...
          <aside class="job-module-menu">
            <template v-for="item in jobSideItems" :key="item">
              <button
                ...
                :aria-expanded="isExpandableJobMenuItem(item) ? isJobMenuExpanded(item) : undefined"
              ...
              <div
                v-if="item === '产业调研'"
                class="job-sub-menu"
                :class="{ open: isJobMenuExpanded('产业调研') }"
                :aria-hidden="!isJobMenuExpanded('产业调研')"
                :inert="!isJobMenuExpanded('产业调研')"
              >
              ...
              <div
                ...
                class="job-sub-menu"
                :class="{ open: isJobMenuExpanded('产业调研报告') }"
                :aria-hidden="!isJobMenuExpanded('产业调研报告')"
                :inert="!isJobMenuExpanded('产业调研报告')"
              >
                <button
                  class="job-sub-button"
            ...
            <div v-if="currentJobSection === '产业调研'" class="job-research-page">
              ...
              <section v-if="!industryResearchDemoInitialized" class="research-uninitialized-state">
                ...
                <div class="research-uninitialized-copy">
                  <span>未初始化</span>
                  <h3>产业调研数据未初始化</h3>
                  <p>请先前往 CMS 进行数据初始化。初始化完成后，这里将展示产业链图谱、区域产业分析、政策库、企业库和岗位分析结果。</p>
                </div>
