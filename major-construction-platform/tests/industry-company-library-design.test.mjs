...
test('industry company library mirrors the Figma list structure in Vue and static entries', () => {
  for (const [label, source] of [
    ...
    assert.match(source, /job-company-flat-content/, `${label} should flatten the content-area background for enterprise library`)
	    ...
	    assert.match(source, /research-compact-ai/, `${label} should use the shared dynamic AI summary strip`)
	    assert.match(source, /data-summary-source/, `${label} should expose whether the summary came from AI or fallback`)
...
test('industry company library uses the Figma-style light blue surface and compact table treatment', () => {
  ...
  assert.match(companyBoardStyles, /grid-template-rows:\s*32px 732px/)
  assert.match(companyBoardStyles, /gap:\s*18px/)
  assert.match(companyBoardStyles, /height:\s*814px/)
  assert.match(companyBoardStyles, /min-height:\s*814px/)
...
test('enterprise library follows the policy breakpoints without global overflow', () => {
  ...
  assert.match(stylesCss, /@media \(max-width:\s*900px\) \{[\s\S]*?\.industry-company-board \{[\s\S]*?grid-template-rows:\s*32px minmax\(732px, 1fr\)/)
