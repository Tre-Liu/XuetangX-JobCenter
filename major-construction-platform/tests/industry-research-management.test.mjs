...
test('industry chain graph defaults to treemap and keeps sankey summary as switchable view', () => {
  ...
  assert.doesNotMatch(appVue, /industry-stage-national-tags/)
  assert.doesNotMatch(stylesCss, /\.industry-stage-national-tags/)
  assert.match(appVue, /class="industry-treemap-stage-copy"/)
  assert.match(appVue, /class="industry-treemap-stage-stat"/)
  assert.match(styleBlock('.industry-treemap-stage header'), /align-items:\s*center/)
  assert.match(styleBlock('.industry-treemap-stage-copy'), /flex:\s*1\s+1\s+auto/)
  assert.match(styleBlock('.industry-treemap-stage header span'), /white-space:\s*nowrap/)
  assert.match(styleBlock('.industry-treemap-stage-stat'), /text-align:\s*right/)
...
test('industry chain graph renders national industry metrics in the Vue page', () => {
  ...
  assert.match(appVue, /class="industry-national-kpi-card industry-figma-kpi-card"/)
  assert.match(appVue, /v-for="metric in NATIONAL_INDUSTRY_CHAIN_METRICS\.summaryMetrics"/)
  assert.match(appVue, /@click="openNationalIndustryMetricDialog\(metric\.label, \$event\)"/)
  ...
  assert.doesNotMatch(appVue, /formatIndustryStageNationalIndustries\(stage\.key\)/)
