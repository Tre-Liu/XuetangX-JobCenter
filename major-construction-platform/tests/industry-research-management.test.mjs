...
test('official major picker supports undergraduate vocational custom search and confirmation', () => {
  assert.match(appVue, /type CmsOfficialMajorLevel = 'undergraduate' \| 'vocational' \| 'custom'/)
  ...
  assert.match(appVue, /自定义专业/)
  assert.match(appVue, /cmsCustomMajorName/)
  assert.match(appVue, /请输入专业名称/)
  assert.match(appVue, /value="custom"/)
  assert.match(appVue, /majorCode = `custom:\$\{customMajorName\}`/)
  assert.match(appVue, /majorEducationLevel = 'custom'/)
...
test('standalone CMS html mirrors AI course creation loop', () => {
  for (const source of [localHtml, rootLocalHtml]) {
    ...
    assert.match(source, /data-major-level="custom"/)
    assert.match(source, /id="customMajorName"/)
    assert.match(source, /请输入专业名称/)
    assert.match(source, /currentMajorLevel === 'custom'/)
    assert.match(source, /courseForm\.majorEducationLevel = 'custom'/)
    assert.match(source, /courseForm\.majorCode = `custom:\$\{customMajorName\}`/)
