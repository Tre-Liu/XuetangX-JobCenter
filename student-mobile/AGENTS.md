...
2026-09-17 用户调整：四类图谱切换统一放在页面内容区，以常驻横向标签展示，兼容小程序和 H5。顶部导航只显示“课程图谱”，不再使用标题下拉；能力详情隐藏切换栏，返回后恢复。覆盖此前图谱顶部标题下拉的约定。

2026-09-17 用户再次调整：取消常驻横向标签，改为左下角悬浮折叠入口；收起显示当前图谱名称，点击向上展开四类图谱，选择后或点击外部收起。入口避开底部画布工具栏，能力详情隐藏。覆盖本日常驻横向标签约定。

2026-09-18 用户调整：知识图谱移除右上角搜索按钮；固定使用已有自定义布局，右上角“自定义视图”为纯文案，无背景、圆角、图标、下拉箭头或切换菜单，不呈现按钮样式。覆盖此前默认/自定义视图切换约定。

2026-09-18 用户调整：素质节点详情沿用能力节点详情的交互方式，点击进入独立详情页，顶部返回和页内返回均回到原图谱，保留视图模式、平移缩放及滚动位置；详情页隐藏图谱类型切换入口。素质详情内容仍使用原有素质数据与空态，不复制能力数据。覆盖此前底部面板方式。

2026-09-18 素质详情视觉补充：完整沿用能力详情的统计卡、知识点与学习内容/拓展资源标签、内容空态、下级节点列表结构，改用淡红背景与红色强调色。没有提供的知识点和学习单元不虚构，当前显示 0 与对应空态。

2026-09-18 问题详情更新：沿用能力/素质详情独立页及统计卡、学习内容与拓展资源标签，额外保留可跳转的关联问题区域；使用青绿色系，区别能力蓝和素质红。详情隐藏图谱切换，返回保持层级与画布视角。保留原有问题说明和关联关系，无内容时显示 0 与空态，覆盖底部面板约定。
